...
static DX_TIMER_BINDING tmr_monitor_memory = {.period = {30, 0}, .name = "tmr_monitor_memory", .handler = monitor_memory_handler};
...
DX_DIRECT_METHOD_BINDING *direct_method_bindings[] = {&dm_light_control, &dm_memory_leak};
DX_GPIO_BINDING *gpio_bindings[] = {&gpio_network_led, &gpio_led};
DX_TIMER_BINDING *timer_bindings[] = {&tmr_publish_message, &tmr_report_properties, &tmr_monitor_memory};
...
static void monitor_memory_handler(EventLoopTimer *eventLoopTimer)
{
    ...
    #define UTC_BUFFER_SIZE 32
    char utcBuffer[UTC_BUFFER_SIZE];
    ...
    if (dx_isAzureConnected()) {
        ...
        if(newMemoryHighWaterMark > memoryHighWaterMark){
            ...
            // Serialize telemetry as JSON
            bool serialization_result = dx_jsonSerialize(msgBuffer, sizeof(msgBuffer), 2, 
                                        DX_JSON_INT, "MemoryHighWaterMark", newMemoryHighWaterMark,
                                        DX_JSON_STRING, "MemoryReportUTC", dx_getCurrentUtc(utcBuffer, UTC_BUFFER_SIZE)); 
